...
[startpoint-cn](https://github.com/dontbealarmed/startpoint-cn)的桌面啟動器:一鍵啟停本地伺服器、首次下載/匯入遊戲資源、打包重定向 APK(Android)/ IPA(iOS)、原生管理面板。
...
先備妥這三樣,以及一個 Android 模擬器(如 MuMu)或 iOS 裝置:

- [安裝檔](https://github.com/dennis96292/startpoint-cn-launcher/releases) `StartPointCNLauncher_0.1.2_x64-setup.exe`
- [遊戲資源 CDN 分卷](https://github.com/dennis96292/.cdn/releases/tag/cdn-1.4.54) `cn-cdn.tar.part.00`~`.05`(約 10GB)
- [World Flipper 國服原始客戶端](https://github.com/dennis96292/startpoint-cn-launcher/releases):Android 用 APK(`V1.8.1.apk`)、iOS 用 IPA(`iOS-1.8.4.ipa`)
...
3. **打包 APK / IPA** — Android 在「打包 APK」選原始 APK、iOS 在「打包 IPA」選原始 IPA,按開始;產出的檔案已把連線重導到上面設定的 host:port 並跳過雷霆登入,裝進模擬器 / iOS 裝置即可進遊戲。
...
產出 `src-tauri/target/release/bundle/nsis/StartPointCNLauncher_0.1.2_x64-setup.exe`。若要同步更新 server 原始碼後再打包,改用 `pwsh scripts/build-release.ps1`(見該腳本說明)。
